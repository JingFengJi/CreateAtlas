...
public class UICenterOnChild : MonoBehaviour
{
	...
	void CenterOnImmediately(Transform target, Vector3 panelCenter)
	{
	...
	public void CenterOnImmediately(Transform target)
	{
		if (mScrollView != null && mScrollView.panel != null)
		{
			Vector3[] corners = mScrollView.panel.worldCorners;
			Vector3 panelCenter = (corners[2] + corners[0]) * 0.5f;
			CenterOnImmediately(target, panelCenter);
		}	
	}

	public void CenterOnImmediately(int index)
	{
		if(index < 0 || index >= transform.childCount) return;
		Transform centerTrans = transform.GetChild(index);
		if(centerTrans != null)
			CenterOnImmediately(centerTrans);
	}
}
