...
public class UILabel : UIWidget
{
	...
	[HideInInspector][SerializeField] float mOutlineProDr = 2f;
	[HideInInspector][SerializeField] bool PanelAlphaControl = false;
    ...
    private static void DynamicFontBrokenFix(Font fnt)
    {
        ...
        // {
            string tmpStr = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!%-+=:',./ ";
            fnt.RequestCharactersInTexture(tmpStr, 150);
        // }
    }
			...
			if (lbl != null)
			{
				...
				if (fnt == font)
				{
					...
					if (!mTempLabels.Contains(lbl))
					{
						mTempLabels.Add(lbl);
						lbl.UpdateNGUIText();
                        fnt.RequestCharactersInTexture(lbl.text, lbl.fontSize,lbl.fontStyle);
                        // DynamicFontBrokenFix(font);
	...
	public override void OnFill (List<Vector3> verts, List<Vector2> uvs, List<Color> cols)
	{
		...
		if (effectStyle != Effect.None)
		{
			...
			if(effectStyle == Effect.OutlinePro)
			{
				float arcLength = 1.4142f;
                float dr = mOutlineProDr;
                float r = pos.x;
			...
			else
			{
				...
				{   
					ApplyShadow(verts, uvs, cols, offset, end, pos.y, -pos.y); // 如果是ShadowAndOutline 模式 x 控制阴影 y 控制描边
				}
				else
				{
				...
				if (effectStyle == Effect.ShadowAndOutline)
					{
						...
						ApplyShadow(verts, uvs, cols, offset, end, -pos.y, pos.y);
						offset = end;
						end = verts.Count;
						ApplyShadow(verts, uvs, cols, offset, end, pos.y, pos.y);
						offset = end;
						...
						ApplyShadow(verts, uvs, cols, offset, end, 0, -pos.x);
					}
	...
	public void ApplyShadow (List<Vector3> verts, List<Vector2> uvs, List<Color> cols, int start, int end, float x, float y)
	{
		...
		for (int i = start; i < end; ++i)
		{
			...
			{
				Color fc = c;
				if(PanelAlphaControl)
					fc.a = uc.a * c.a * panel.alpha;
				else
					fc.a = uc.a * c.a;
				cols[i] = fc;
			}
